...
## 线上启用 Dify 转 DeepSeek

如果线上运行状态显示当前模型为 `local`，通常不是代码没有接入外部模型，而是启动服务时没有带上外部模型环境变量。路由器没有检测到可用的 provider 时，会自动回退到本地规则。

推荐走 `Dify -> DeepSeek` 路线：本服务只连接 Dify，DeepSeek API Key 放在 Dify 应用里管理。这样本项目不用直接保存 DeepSeek Key，也方便在 Dify 后台继续调提示词。

在服务器启动服务前，先在同一个终端设置环境变量：

```bash
export COMPANION_LLM_PROVIDER='dify'
export DIFY_API_KEY='your-dify-api-key'
export DIFY_API_BASE_URL='https://api.dify.ai/v1'
export DIFY_RESPONSE_MODE='blocking'
export DIFY_APP_USER_PREFIX='wechat-treehole'
```
...
```bash
curl -s http://127.0.0.1:8765/api/llm-router/status
```
...
- `DIFY_API_KEY` 是否是 Dify 应用的 App API Key，而不是 DeepSeek API Key
- 启动服务的终端是否真的执行过上面的 `export`
- 如果使用 `systemd`、`pm2`、Docker 或后台脚本启动，环境变量是否写进对应的服务配置里
- 修改环境变量后是否已经重启服务
