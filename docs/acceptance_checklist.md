...
| Main navigation | 聊天模拟、用户、记忆、微信入口、样本状态、运行状态 are visible and clickable | To review |
...
| WeChat entry mock | The 微信入口 tab can simulate inbound text and show an outbound envelope | To review |
| Public source status | 样本状态 shows source names, license, download policy, and readiness status | Observed, wording fixed |
| Run status | 运行状态 shows service status, database path, and deferred media assets | Observed, wording fixed |
