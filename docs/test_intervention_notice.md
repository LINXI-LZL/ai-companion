...
| Why now | The local app runs, the simulator can send messages, and the core safety, memory, sticker intent, voice intent, sample status, and admin views are wired |
...
| Area | Reason |
|---|---|
| Real WeChat send/receive | WeChat adapter remains deferred until API credentials and current docs are reviewed |
