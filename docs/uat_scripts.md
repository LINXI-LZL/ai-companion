...
| Expected result | The page opens without a browser error. The sidebar shows 聊天模拟、用户、记忆、微信入口、样本状态、运行状态. Each tab switches to its own screen. |
...
| Preconditions | Local console is open on 聊天模拟 |
| Steps | 1. In the message box, type `老板又临下班改需求，真的离谱`. 2. Click Send. 3. Look at the reply and the Latest Plan panel. |
| Expected result | The reply feels like a sharp but supportive friend. 模式 shows text plus sticker intent. 表情 shows speechless or similar reaction. The reply should not repeat a previous unrelated answer. |
...
| Expected result | 模式 shows text plus short voice script. 语音 shows sleepy companion. The voice script area contains a short script. The actual audio file is not expected yet. |
...
| Expected result | 安全 is on. The reply is serious and grounding. 表情 and playful voice should be none. The reply should not tease the user. |
...
| Steps | 1. Open 记忆. 2. Save `用户喜欢短回复`. 3. Go back to 聊天模拟. 4. Send `今天又有点emo`. |
...
| Steps | 1. Open 微信入口. 2. Keep external user as `external-owner`. 3. Send `老板又临下班改需求，真的离谱`. 4. Read 适配结果. |
| Expected result | 适配结果 shows channel `wecom_kf`, content type `文字`, an outbound text reply, and send policy `仅本地模拟`. This proves the local adapter path works, not real WeChat delivery. |
...
| Preconditions | Local console is open |
| Steps | 1. Open 样本状态. 2. Confirm sources appear. 3. Open 运行状态. 4. Confirm service status and media assets appear. |
| Expected result | 样本状态 lists public sources with license and download policy. 运行状态 shows service status, database path, and deferred media assets. |
